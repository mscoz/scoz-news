#!/usr/bin/env python3
"""
SCOZ News — Build HTML from canonical data/*.json editions
Design: HAZE (glassmorphism + category-reactive background)
Executar: python3 build_html.py
"""
import json, os, html, sys, tempfile
...
def publish_outputs(html_path, html_out, cache_path, cache):
...
def main():
    ...
    html_out = build_html(data, make_logo_b64())
    cache = build_cache(data)
    publish_outputs(out_path, html_out, CACHE_PATH, cache)

    weeks = data["weeks"]
    total = sum(
        sum(len(week["items"][category_id]) for category_id, *_ in CATEGORIES)
        for week in weeks
    )
    print(f"OK — {len(weeks)} edição(ões), {total} notícias → {out_path}")
